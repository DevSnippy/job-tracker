// AI Tailor flow — the headline feature.
// Flow: fetch JD → extract requirements → ask user clarifying Qs → produce tailored resume → preview + export.
const { useState: uST, useEffect: uET, useMemo: uMT } = React;
...
const TailorFlow = ({ jobId, initialStep = null, onClose, onNavigateResume }) => {
  const { JOBS } = window.JT_DATA;
  // synthetic "Elementor" job if id points there
  const job = jobId === "elementor-fs" ? {
    ...
    source: "https://elementor.com/careers/position/full-stack-developer",
  } : (JOBS.find(j => j.id === jobId) || JOBS[0]);

  const initIdx = initialStep ? Math.max(0, TAILOR_STEPS.findIndex(s=>s.key===initialStep)) : 0;
  const [stepIdx, setStepIdx] = uST(initIdx);
  const step = TAILOR_STEPS[stepIdx];

  // fetch sim
  const [fetchLog, setFetchLog] = uST([]);
  uET(() => {
    if (step.key !== "fetch") return;
    const lines = [
      `→ GET ${job.source || "techmap://" + job.id}`,
  ...
  const analysis = {
    ...
    tone: "Collaborative, craft-focused, ships weekly",
  };

  // QA state
  const [answers, setAnswers] = uST({
    php_years: "",
    wordpress: "",
    headline: "",
    emphasis: [],
  ...
  const toggleEmphasis = (v) => setAnswers(a => ({
    ...a,
    emphasis: a.emphasis.includes(v) ? a.emphasis.filter(x=>x!==v) : [...a.emphasis, v],
  }));

  // generation
  const [generated, setGenerated] = uST(false);
  uET(() => {
  ...
  const canAdvance =
    step.key === "fetch" ? false :
    step.key === "qa"    ? (answers.php_years && answers.wordpress) :
    true;

  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 100, display: "grid", placeItems: "center" }} onClick={onClose}>
      <div className="panel" style={{ width: "min(1120px, 94vw)", height: "86vh", display: "flex", flexDirection: "column", boxShadow: "var(--shadow-lg)" }} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ padding: "10px 18px", borderBottom: "1px solid var(--border)", background: "var(--surface-2)" }}>
          <div className="row" style={{ gap: 6 }}>
            {TAILOR_STEPS.map((s, i) => (
              <React.Fragment key={s.key}>
                <div className="row" style={{ gap: 6, color: i<=stepIdx?"var(--text)":"var(--text-3)" }}>
                  <span style={{
                    width: 18, height: 18, borderRadius: "50%",
                    background: i<stepIdx?"var(--accent)":i===stepIdx?"var(--accent-soft)":"var(--surface-3)",
                    color: i<stepIdx?"var(--text-inv)":i===stepIdx?"var(--accent)":"var(--text-3)",
                    display:"grid", placeItems:"center", fontSize: 11, fontWeight: 600
                  }}>{i<stepIdx?"✓":i+1}</span>
                  <span style={{ fontSize: 12.5, fontWeight: i===stepIdx?500:400 }}>{s.label}</span>
                </div>
                {i < TAILOR_STEPS.length-1 && <span style={{ flex: 1, height: 1, background: i<stepIdx?"var(--accent)":"var(--border)" }}/>}
              </React.Fragment>
            ))}
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflow: "auto", padding: 20 }}>
          {step.key === "fetch" && (
            <div style={{ maxWidth: 720, margin: "40px auto" }}>
              ...
              <div className="panel mono" style={{ padding: 14, background: "var(--surface-2)", fontSize: 11.5, minHeight: 180 }}>
                <div className="muted">$ jobtracker fetch --url={job.source || job.id}</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
              <div className="panel" style={{ padding: 14 }}>
                ...
                <div style={{ fontSize: 12.5, lineHeight: 1.65, color: "var(--text-2)", maxHeight: 420, overflow: "auto" }}>
                  ...
                  <p><strong style={{color:"var(--text)"}}>What you'll do.</strong></p>
                  <ul><li>Design and ship full-stack features (PHP + React)</li><li>Collaborate with product + design on complex editor UX</li><li>Work with MySQL at scale; profile and optimize queries</li><li>Design REST APIs consumed by the editor and third-party plugins</li><li>Improve CI/CD + test coverage across the codebase</li></ul>
              ...
              <div className="col" style={{ gap: 14 }}>
                <div className="panel" style={{ padding: 14 }}>
                  ...
                  <ul style={{ margin: 0, paddingLeft: 16, lineHeight: 1.75, fontSize: 12.5 }}>
                    {analysis.must.map(r => <li key={r}>{r}</li>)}
                  </ul>
                </div>
                <div className="panel" style={{ padding: 14 }}>
                  ...
                  <ul style={{ margin: 0, paddingLeft: 16, lineHeight: 1.75, fontSize: 12.5, color: "var(--text-2)" }}>
                    {analysis.nice.map(r => <li key={r}>{r}</li>)}
                  </ul>
                </div>
                <div className="panel" style={{ padding: 14 }}>
          ...
          {step.key === "qa" && (
            <div style={{ maxWidth: 720, margin: "0 auto" }}>
              ...
              <div className="muted" style={{ marginBottom: 20, fontSize: 12.5 }}>
                The JD calls out specifics your default resume doesn't cover. Answer these and Claude will weave them into your tailored version.
              </div>

              <Q label="How many years of production PHP do you have?" subtitle="They list 5+ years full-stack, PHP-weighted.">
                <div className="row" style={{ gap: 4, flexWrap: "wrap" }}>
                  {["< 2","2–4","5–7","8+"].map(o => (
                    <button key={o} className={`btn sm ${answers.php_years===o?"primary":""}`} onClick={()=>setAns("php_years", o)}>{o}</button>
                  ))}
                </div>
              </Q>
              ...
              <Q label="WordPress ecosystem experience?" subtitle="Nice-to-have — bump it up if you've got it.">
                <div className="row" style={{ gap: 4, flexWrap: "wrap" }}>
                  {["None","Hobbyist","Shipped plugins/themes","Core contributor"].map(o => (
                    <button key={o} className={`btn sm ${answers.wordpress===o?"primary":""}`} onClick={()=>setAns("wordpress", o)}>{o}</button>
                  ))}
                </div>
              </Q>

              <Q label="Any of these to emphasize in the summary?" subtitle="Pick as many as apply.">
                <div className="row" style={{ gap: 4, flexWrap: "wrap" }}>
              ...
              <Q label="Tailored headline" subtitle="What should sit under your name? (Optional — we'll propose one.)">
              ...
              <Q label="Anything else we should weave in?" subtitle="A specific project, a niche skill, a tone preference…">
                <textarea className="input" rows={3} placeholder="e.g. Led a MySQL migration that halved p95 query time. Prefer concise first-person." value={answers.extra} onChange={e=>setAns("extra", e.target.value)}/>
              </Q>

              <div className="muted" style={{ fontSize: 11.5, marginTop: 14 }}>
                <Icon name="zap" size={11}/> Skip any — we'll fill gaps conservatively from your default resume.
              </div>
            </div>
          )}
              ...
              {!generated ? (
                <>
                  ...
                  <div className="panel mono" style={{ padding: 12, textAlign: "left", background: "var(--surface-2)", fontSize: 11.5 }}>
                    ...
                    <div>→ applying answers from Q&A (5 inputs)</div>
          ...
          {step.key === "review" && (
            <TailorReview job={job} answers={answers} />
          )}
        </div>
        ...
        <div style={{ padding: "12px 16px", borderTop: "1px solid var(--border)", display: "flex", justifyContent: "space-between" }}>
          ...
          <div className="row" style={{ gap: 6 }}>
            {stepIdx > 0 && stepIdx < TAILOR_STEPS.length - 1 && step.key !== "fetch" && (
...
const Q = ({ label, subtitle, children }) => (
...
const TailorReview = ({ job, answers }) => {
  ...
  const before = {
    ...
    bullets: [
      "Led migration to a tokenized design system across 40+ surfaces.",
      "Rebuilt the large-dataset table layer — p95 interaction 420ms → 90ms.",
      "Mentored three mid-level engineers; set code-review culture.",
    ],
  };
  const after = {
    headline: answers.headline || "Full-stack Engineer · PHP + React at scale",
    summary: `Full-stack engineer with 7+ years shipping production web apps on PHP and React, comfortable owning both a relational data layer and the editor UI that sits on top of it.${answers.wordpress && answers.wordpress !== "None" ? " " + answers.wordpress + " in the WordPress ecosystem." : ""}${answers.emphasis.length ? " Strengths: " + answers.emphasis.slice(0,3).join(", ").toLowerCase() + "." : ""}`,
    skills: ["PHP","React","MySQL","Node.js","REST API","TypeScript","WordPress","CI/CD","Docker","Perf"],
    bullets: [
      "Architected a PHP + React editor surface used by 18M+ creators weekly; shipped features end-to-end.",
      "Optimized MySQL queries powering the rule engine — p95 halved from 420ms → 90ms.",
      "Designed REST APIs consumed by the editor and third-party plugins; owned versioning + deprecations.",
      "Mentored three mid-level engineers; set code-review culture across the full stack.",
    ],
  };

  const DiffLine = ({ old: o, now: n }) => (
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: 18 }}>
      <div className="col" style={{ gap: 14 }}>
        ...
        <div>
          ...
          <DiffLine old={before.headline} now={after.headline}/>
        </div>

        <div>
          <h4 className="section-title">Summary</h4>
          <DiffLine old={before.summary} now={after.summary}/>
        </div>

        <div>
          <h4 className="section-title">Skills reordered</h4>
          <div className="row" style={{ gap: 4, flexWrap: "wrap", marginBottom: 6 }}>
            {before.skills.map(s => <span key={s} className="chip" style={{ opacity: 0.5, textDecoration: "line-through" }}>{s}</span>)}
          </div>
          <div className="row" style={{ gap: 4, flexWrap: "wrap" }}>
            {after.skills.map(s => <span key={s} className={`chip ${before.skills.includes(s)?"":"chip-accent"}`}>{s}</span>)}
          </div>
        </div>
        ...
        <div>
          ...
          {after.bullets.map((b, i) => (
            <DiffLine key={i} old={before.bullets[i] || "(new bullet)"} now={b}/>
          ))}
        </div>
      </div>
      ...
      <aside className="col" style={{ gap: 12 }}>
        <div className="panel" style={{ padding: 14 }}>
          ...
          <div className="stack-sm">
            {[
              { k: "PHP",      before: 10, after: 92 },
              { k: "React",    before: 88, after: 95 },
              { k: "MySQL",    before: 20, after: 88 },
              { k: "REST API", before: 55, after: 90 },
              { k: "WordPress",before: 0,  after: answers.wordpress && answers.wordpress !== "None" ? 70 : 20 },
            ].map(r => (
              <div key={r.k}>
                ...
                <div style={{ height: 4, background: "var(--surface-2)", borderRadius: 2, marginTop: 3, position: "relative" }}>
                  ...
                  <div style={{ position: "absolute", left: 0, top: 0, width: `${r.after}%`, height: "100%", background: "var(--accent)", borderRadius: 2, opacity: 0.9 }}/>
                </div>
              </div>
            ))}
          </div>
...
Object.assign(window, { TailorFlow });
